...
import './github-eci.css'
import { ResponsiveBump } from '@nivo/bump'
import { useEffect } from 'react';
import colormap  from 'colormap';
import Select from 'react-select'
import SourceData from './data/github_eci_1.json'
...
for (let [key,roundItem] of Object.entries(SourceData)){
  roundItem = roundItem.map(item=>{
    item[0] = item[0] + ` ${countryMap[item[0]]}`
    return item
  })
  selectOpt.push({value:sourceArray.length, label:key})
  sourceArray.push(roundItem)
}
...
const countryEmphasize = ['南非','韩国','中国','新加坡','西班牙','波兰','伊朗','瑞典', '土耳其',"中国台湾","丹麦","俄罗斯"]
...
function App() {
  ...
  return (
      ...
      IPC Level 3 复杂度， 产品总数为 4485
            ...
            {/* <div>default round 4</div> */}
            <p style={{fontSize:"1.6em"}}>知识复杂度（Github）</p>
